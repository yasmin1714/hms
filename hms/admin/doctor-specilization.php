...
if (strlen($_SESSION['id'] == 0)) {
    ...
    if (isset($_POST['submit'])) {
        ...
        if(empty($doctorspecilization)){
             $_SESSION['msg'] = "Doctor Specialization cannot be empty!";
        }else{
            $sql = mysqli_query($con, "insert into doctorspecilization(specialization) values('$doctorspecilization')");
            if ($sql) {
    ...
    if (isset($_GET['del'])) {
        $sid = $_GET['id'];
        $sqldel = mysqli_query($con, "delete from doctorspecilization where id = '$sid'");
        if ($sqldel) {
